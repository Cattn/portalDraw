...
export class Database {
	...
	async getBoards(): Promise<Board[]> {
		return new Promise((resolve, reject) => {
			this.db.all('SELECT * FROM boards ORDER BY created_at DESC', (err, rows) => {
				if (err) reject(err);
				...
				(err, rows: any[]) => {
					if (err) reject(err);
						...
						const sessions = rows.map((row) => ({
							...
							cursor:
								row.cursor_x && row.cursor_y ? { x: row.cursor_x, y: row.cursor_y } : undefined
						}));
						resolve(sessions);
					}
				}
	...
	async removeSession(sessionId: string): Promise<void> {
		return new Promise((resolve, reject) => {
			this.db.run('DELETE FROM board_sessions WHERE id = ?', [sessionId], (err) => {
				if (err) reject(err);
				else resolve();
			});
		});
